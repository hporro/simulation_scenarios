...
Event::Event(Event_IDs evid) : m_id(evid) {}

WindowResizeEvent::WindowResizeEvent(int width, int height) : m_width(width), m_height(height), Event(WindowResize) {}

ButtonEvent::ButtonEvent(int key, int scancode, int action, int mods) : m_key(key), m_scancode(scancode), m_action(action), m_mods(mods), Event(Button) {}
...
void Application::setWidthHeight(int width, int height) {
	m_width = width;
	m_height = height;
}
...
void Application::setup_callbacks() {
	...
	glfwSetKeyCallback(window,
		[](GLFWwindow* window, int key, int scancode, int action, int mods) {
			...
			app->eventQueue.push_back(new ButtonEvent(key, scancode, action, mods));
		}
	);
	glfwSetWindowSizeCallback(window, 
		[](GLFWwindow* window, int width, int height) {
			auto app = (Application*)glfwGetWindowUserPointer(window);
			app->eventQueue.push_back(new WindowResizeEvent{ width, height });
		});
